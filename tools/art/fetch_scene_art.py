"""Fetches journey backdrops from an online image generator.
    ...
    python.exe tools\\art\\fetch_scene_art.py --out-dir journey2 --salt journey2 \
        --only SC-A,SC-B                                     # fresh alt set, same seeds again
...
DEFAULT_OUT_DIR = ROOT / "game" / "assets" / "art" / "backgrounds" / "journey"
...
def seed_for(scene_id: str, salt: str = "") -> int:
    """Stable per scene, so the art does not change under you on a rebuild.
    A salt gives a fresh but still reproducible set for a second run."""
    return zlib.crc32((scene_id + salt).encode("utf-8")) & 0x7FFFFFFF
...
def main() -> None:
    ...
    parser.add_argument("--only", help="fetch scenes by id, comma-separated")
    ...
    parser.add_argument("--out-dir", default=str(DEFAULT_OUT_DIR),
                        help="write backdrops here instead of the shipped journey dir")
    parser.add_argument("--salt", default="",
                        help="string mixed into the per-scene seed; use it to get "
                             "a fresh but still reproducible set of images")
    ...
    wanted = ([s.strip() for s in args.only.split(",") if s.strip()]
              if args.only else sorted(scenes))
    ...
    for i, scene_id in enumerate(wanted):
        ...
        try:
            image = fetch(prompt, seed_for(scene_id, args.salt), args.token)
    ...
    # should never require reconstructing what was asked for. Only for a full
    # run into the shipped directory - a subset fetched elsewhere would write
    # seeds here that do not match the files anyone is looking at.
    if out_dir == DEFAULT_OUT_DIR and not args.only:
